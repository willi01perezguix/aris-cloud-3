...
class AdminAdapter:
    ...
    def create_tenant(self, name: str) -> dict:
        return self.tenants.create(name=name)

    def list_stores(self, tenant_id: str) -> list[Store]:
        return [Store(**item) for item in self.stores.list(tenant_id=tenant_id)]

    def create_store(self, tenant_id: str, name: str) -> dict:
        return self.stores.create(tenant_id=tenant_id, name=name)

    def list_users(self, tenant_id: str) -> list[User]:
        return [User(**item) for item in self.users.list(tenant_id=tenant_id)]

    def create_user(self, tenant_id: str, email: str, password: str) -> dict:
        return self.users.create(tenant_id=tenant_id, email=email, password=password)
