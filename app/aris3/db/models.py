...
class PurgeLock(Base):
    __tablename__ = "purge_locks"
    ...
    __table_args__ = (UniqueConstraint("resource_type", "resource_id", name="uq_purge_locks_resource"),)
...
class ExportRecord(Base):
    ...
    generated_by_user_id: Mapped[uuid.UUID | None] = mapped_column(GUID(), nullable=True)
...
class PosAdvance(Base):
    ...
    voucher_type: Mapped[str] = mapped_column(String(20), nullable=False, default="ADVANCE")
    ...
    issued_cash_movement_id: Mapped[uuid.UUID | None] = mapped_column(GUID(), ForeignKey("pos_cash_movements.id"), nullable=True, index=True)
...
class PosCashMovement(Base):
    ...
    cashier_user_id: Mapped[uuid.UUID] = mapped_column(GUID(), index=True, nullable=False)
    actor_user_id: Mapped[uuid.UUID | None] = mapped_column(GUID(), index=True, nullable=True)
    business_date: Mapped[date] = mapped_column(Date, nullable=False)
    timezone: Mapped[str] = mapped_column(String(64), nullable=False, default="UTC")
...
class PosCashDayClose(Base):
    ...
    expected_cash: Mapped[float] = mapped_column(nullable=False, default=0.0)
    counted_cash: Mapped[float | None] = mapped_column(nullable=True)
    difference_amount: Mapped[float | None] = mapped_column(nullable=True)
    difference_type: Mapped[str | None] = mapped_column(String(10), nullable=True)
    net_cash_sales: Mapped[float] = mapped_column(nullable=False, default=0.0)
    cash_refunds: Mapped[float] = mapped_column(nullable=False, default=0.0)
    net_cash_movement: Mapped[float] = mapped_column(nullable=False, default=0.0)
    day_close_difference: Mapped[float | None] = mapped_column(nullable=True)
