...
class ExportRepository:
    ...
    def list_by_tenant(self, tenant_id: str, *, store_id: str | None = None) -> list[ExportRecord]:
        stmt = select(ExportRecord).where(ExportRecord.tenant_id == tenant_id)
        if store_id:
            stmt = stmt.where(ExportRecord.store_id == store_id)
        return self.db.execute(stmt.order_by(ExportRecord.created_at.desc(), ExportRecord.id)).scalars().all()
