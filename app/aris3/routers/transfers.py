...
def _transfer_line_response(line: TransferLine) -> TransferLineResponse:
...
def _movement_summary(repo: TransferRepository, transfer: Transfer) -> dict:
    dispatched_lines, dispatched_qty = repo.count_dispatched(_normalize_uuid(transfer.id))
    pending_reception = transfer.status == "DISPATCHED"
    return {
...
def _transfer_response(repo: TransferRepository, transfer: Transfer, lines: list[TransferLine]) -> TransferResponse:
    ...
    return TransferResponse(
        header=header,
        lines=[_transfer_line_response(line) for line in lines],
        movement_summary=_movement_summary(repo, transfer),
    )
