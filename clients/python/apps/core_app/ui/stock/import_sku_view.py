...
from ui.shared.validators import validate_import_sku_lines
...
class ImportSkuView:
    ...
    def submit(self, lines: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        if not self.list_view.can_import_sku():
            self.telemetry.emit(build_event(category="permission_denied", name="permission_denied", module="stock", action="import_sku"))
            return {"ok": False, "error": "You do not have permission to import SKU stock"}
        validation = validate_import_sku_lines(lines)
        ...
        try:
            outcome: StockMutationResult = self.service.import_sku(lines)
