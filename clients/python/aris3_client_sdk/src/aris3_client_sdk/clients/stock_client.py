...
from ..idempotency import IdempotencyKeys, idempotency_headers, new_idempotency_keys
...
class StockClient(BaseClient):
    ...
    def import_epc(
        ...
        if not lines:
            raise ClientValidationError(
                [ValidationIssue(row_index=None, field="lines", reason="lines must not be empty")]
            )
        normalized = [validate_import_epc_line(line, idx) for idx, line in enumerate(lines)]
        keys = self.idempotency_keys(transaction_id, idempotency_key)
    ...
    def idempotency_keys(
        ...
        return _resolve_idempotency(transaction_id=transaction_id, idempotency_key=idempotency_key)
