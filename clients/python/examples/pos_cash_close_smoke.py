...
def main() -> None:
    parser = argparse.ArgumentParser(description="Close a POS cash session (smoke)")
    parser.add_argument("--env-file", default=None)
    parser.add_argument("--store-id", required=True)
    parser.add_argument("--counted-cash", type=float, required=True)
    ...
    try:
        response = client.cash_action("CLOSE", payload, idempotency_key=keys.idempotency_key)
    except ApiError as exc:
        print(json.dumps({"error": exc.code, "message": exc.message, "trace_id": exc.trace_id}, indent=2))
        raise SystemExit(1) from exc

    print(f"Cash session closed: {response.id} status={response.status}")
