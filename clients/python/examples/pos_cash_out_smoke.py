...
def main() -> None:
    parser = argparse.ArgumentParser(description="Cash out for POS cash session (smoke)")
    ...
    try:
        response = client.cash_action("CASH_OUT", payload, idempotency_key=keys.idempotency_key)
    except ApiError as exc:
        print(json.dumps({"error": exc.code, "message": exc.message, "trace_id": exc.trace_id}, indent=2))
        raise SystemExit(1) from exc

    print(f"Cash out recorded: {response.id} expected_cash={response.expected_cash}")
