...
def main() -> None:
    parser = argparse.ArgumentParser(description="Close a POS cash business day (smoke)")
    parser.add_argument("--env-file", default=None)
    parser.add_argument("--store-id", required=True)
    parser.add_argument("--business-date", required=True)
    parser.add_argument("--timezone", required=True)
    parser.add_argument("--counted-cash", type=float)
    parser.add_argument("--force-if-open-sessions", action="store_true")
    ...
    payload = {
        ...
        "force_if_open_sessions": args.force_if_open_sessions,
        "reason": args.reason,
    }
    if args.counted_cash is not None:
        payload["counted_cash"] = args.counted_cash

    try:
        response = client.day_close(payload, idempotency_key=keys.idempotency_key)
    except ApiError as exc:
        print(json.dumps({"error": exc.code, "message": exc.message, "trace_id": exc.trace_id}, indent=2))
        raise SystemExit(1) from exc

    print(f"Day close status: {response.status} closed_at={response.closed_at}")
