...
from aris3_client_sdk.stock_validation import ValidationIssue, validate_import_epc_line
...
def _validate_lines(lines: list[dict]) -> list[dict]:
    ...
    for idx, line in enumerate(lines):
        try:
            normalized.append(validate_import_epc_line(line, idx).model_dump(mode="json", exclude_none=True))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="ARIS3 stock import-epc smoke CLI")
    ...
    try:
        ...
        response = client.import_epc(normalized, transaction_id=keys.transaction_id, idempotency_key=keys.idempotency_key)
