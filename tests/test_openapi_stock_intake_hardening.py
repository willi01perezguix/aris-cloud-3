...
def test_intake_mutations_document_idempotency_key_header():
    openapi = app.openapi()
    for path, method in IDEMPOTENCY_DOCUMENTED_ENDPOINTS:
...
def test_preload_issue_mutations_do_not_document_idempotency_key_header_when_not_enforced():
    openapi = app.openapi()
    for path, method in MUTATING_ENDPOINTS_WITHOUT_IDEMPOTENCY:
        operation = openapi["paths"][path][method]
        params = operation.get("parameters", [])
        idempotency = next((param for param in params if param["name"] == "Idempotency-Key"), None)
